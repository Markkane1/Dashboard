...
export default function Navbar() {
  ...
  const navItems = useMemo<NavItem[]>(() => [
    { href: "/dashboard", label: tc("dashboard"), show: isAuthenticated },
    { href: "/courses", label: tc("courses"), show: true },
    { href: "/notifications", label: t("notifications"), show: isAuthenticated },
    { href: "/instructor/content", label: t("content"), show: isAuthenticated && canManageContent },
    { href: "/instructor/analytics", label: t("analytics"), show: isAuthenticated && canManageContent },
    { href: "/admin", label: t("admin"), show: isAuthenticated && isAdmin },
  ], [canManageContent, isAdmin, isAuthenticated, t, tc]);
      ...
      <div className="app-shell">
        <div className="flex min-h-14 items-center justify-between gap-4">
          <Link href={isAuthenticated ? "/dashboard" : "/"} className="flex min-w-0 items-center gap-2">
            ...
            <span className="truncate text-sm font-black text-slate-950 sm:text-base">{t("appName")}</span>
          </Link>
              ...
              {isAuthenticated ? (
                <div className="flex flex-col gap-2">
                  <p className="px-3 text-sm font-semibold text-slate-600">
                    {tc("signedInAs")} <span className="font-bold text-slate-950">{session?.user?.name}</span>
                  </p>
                  <button
                    onClick={() => {
                      setIsMobileMenuOpen(false);
